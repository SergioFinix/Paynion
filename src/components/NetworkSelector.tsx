"use client";
import Image from "next/image";
import { useState } from "react";

const networks = [
  { value: "ethereum", label: "Ethereum", logo: "/networks/ethereum.svg" },
  { value: "polygon", label: "Polygon", logo: "/networks/polygon.svg" },
  { value: "arbitrum", label: "Arbitrum", logo: "/networks/arbitrum.svg" },
];

export default function NetworkSelector({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const [open, setOpen] = useState(false);
  const selected = networks.find(n => n.value === value);

  return (
    <div className="relative">
      <button
        type="button"
        className="w-full flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-400"
        onClick={() => setOpen(!open)}
      >
        {selected ? (
          <>
            <Image src={selected.logo} alt={selected.label} width={24} height={24} />
            <span>{selected.label}</span>
          </>
        ) : (
          <span className="text-gray-400">Select network</span>
        )}
        <span className="ml-auto">▼</span>
      </button>
      {open && (
        <div className="absolute z-10 mt-2 w-full bg-white border rounded-lg shadow-lg">
          {networks.map(n => (
            <button
              key={n.value}
              type="button"
              className="flex items-center gap-2 w-full px-4 py-2 hover:bg-indigo-50"
              onClick={() => {
                onChange(n.value);
                setOpen(false);
              }}
            >
              <Image src={n.logo} alt={n.label} width={24} height={24} />
              <span>{n.label}</span>
            </button>
          ))}
        </div>
      )}
    </div>
  );
}